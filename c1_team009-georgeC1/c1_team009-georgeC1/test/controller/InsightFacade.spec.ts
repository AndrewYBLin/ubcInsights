...
describe("InsightFacade", function () {
	...
	describe("AddDataset", function () {
		...
		it("should reject with invalid content", async function () {
			try {
				await facade.addDataset("validId", null as unknown as string, InsightDatasetKind.Sections);
				expect.fail("Should have thrown!");
			} catch (err) {
				expect(err).to.be.an.instanceOf(InsightError);
			}
		...
		it("should add valid dataset", async function () {
			try {
				...
				const content = await zip.generateAsync({ type: "base64" });
				const datasetIds = await facade.addDataset("validId", content, InsightDatasetKind.Sections);
		...
		it("should add multiple valid datasets", async function () {
			try {
				...
				const datasetIds1 = await facade.addDataset("dataset1", content1, InsightDatasetKind.Sections);
				...
				const datasetIds2 = await facade.addDataset("dataset2", content2, InsightDatasetKind.Sections);
		...
		it("should load persisted dataset from disk", async function () {
			try {
				...
				expect(datasetList).to.deep.include.members([{ id: "validId", kind: InsightDatasetKind.Sections, numRows: 1 }]);
			} catch (err) {
				expect.fail("Should not have thrown!");
			}
		});
	...
	describe("RemoveDataset", function () {
		...
		it("should remove an existing dataset", async function () {
			try {
				...
				courses1?.file("course1.txt", JSON.stringify(course1));
				const content1 = await zip1.generateAsync({ type: "base64" });
				await facade.addDataset("dataset1", content1, InsightDatasetKind.Sections);
				...
				courses2?.file("course2.txt", JSON.stringify(course2));
				const content2 = await zip2.generateAsync({ type: "base64" });
				await facade.addDataset("dataset2", content2, InsightDatasetKind.Sections);
	...
	describe("PerformQuery", function () {
		...
		it("should reject non-object filter value", async function () {
			try {
				...
				expect(err).to.have.property("message").that.includes("Filter is not an object");
				expect(err).to.be.an.instanceOf(InsightError);
			}
		});
		...
		it("should reject filter that is not exactly one key", async function () {
			try {
				...
				expect(err).to.have.property("message").that.includes("Filter must have exactly one key");
				expect(err).to.be.an.instanceOf(InsightError);
			}
		});
		...
		it("should reject AND with non-array value", async function () {
			try {
				...
				expect(err).to.have.property("message").that.includes("non-empty array");
				expect(err).to.be.an.instanceOf(InsightError);
			}
		});
		...
		it("should reject invalid filter type", async function () {
			try {
				...
				expect(err).to.have.property("message").that.includes("Invalid filter type");
				expect(err).to.be.an.instanceOf(InsightError);
			}
		});
		...
		it("should reject Comparison with non-object content", async function () {
			try {
				...
				expect(err).to.have.property("message").that.includes("Comparison content");
				expect(err).to.be.an.instanceOf(InsightError);
			}
		});
		...
		it("should reject Comparison that is not exactly one key", async function () {
			try {
				...
				expect(err).to.have.property("message").that.includes("Comparison must have exactly one key");
				expect(err).to.be.an.instanceOf(InsightError);
			}
		});
		...
		it("should reject MComparison with non-number value", async function () {
			try {
				...
				expect(err).to.have.property("message").that.includes("MComparison value must be a number");
				expect(err).to.be.an.instanceOf(InsightError);
			}
		});
		...
		it("should reject Comparison with invalid key format", async function () {
			try {
				...
				expect(err).to.have.property("message").that.includes("Invalid Comparison key format");
				expect(err).to.be.an.instanceOf(InsightError);
			}
		});
		...
		it("should reject invalid MComparison keys", async function () {
			try {
				...
				expect(err).to.have.property("message").that.includes("Invalid Comparison key field");
				expect(err).to.be.an.instanceOf(InsightError);
			}
		});
		...
		it("Should reject OPTIONS with extra keys", async function () {
			try {
				...
				expect(err).to.have.property("message").that.includes("Unexpected OPTIONS key found");
				expect(err).to.be.an.instanceOf(InsightError);
			}
		});
		...
		it("Should reject query with multiple IDs", async function () {
			try {
				...
				expect(err).to.have.property("message").that.includes("multiple datasets");
				expect(err).to.be.an.instanceOf(InsightError);
			}
		});
		...
		it("should filter sections with wildcard at start", async function () {
			...
			expect(result).to.deep.equal([{ ubc_dept: "CPSC" }, { ubc_dept: "CPSC" }]);
		});
		...
		it("should filter sections with wildcard at end", async function () {
			...
			expect(result).to.deep.equal([{ ubc_dept: "CPSC" }, { ubc_dept: "COGS" }]);
		});
